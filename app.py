...
def api_download():
    ...
    try:
        ...
        downloaded_files = glob.glob(os.path.join(temp_dir, "*"))
        if not downloaded_files:
            raise Exception("No output file generated by FFmpeg")
        ...
        formats = info.get("formats", []) or []
        combined_fmt = next((f for f in formats if f.get("has_audio") and f.get("has_video") and f.get("url")), None)
        fallback_url = combined_fmt.get("url") if combined_fmt else (info.get("url") or (formats[-1].get("url") if formats else None))
        if fallback_url:
